//Lista duplamente encadeada - First In, First Out
...
void menu ()
{
    ...
    do {
        ...
        switch (op) {
            case 1: inserir (&topo, &ult); break;
            case 2: retirar (&ult); break;
            case 3: listar (topo); break;
        }
    } while (op != 0);
}

void inserir (noPtr * copiaTopo, noPtr * copiaUlt) // inserir no ínico da lista
{
    noPtr p = new no;
...
void retirar (noPtr * copiaUlt)
{
    noPtr p = (*copiaUlt);
    
    if (!listaVazia (*copiaUlt)) {
        if (p->ant == NULL) { //caso haja apenas um elemento na lista
            *copiaUlt = NULL;
        }
        else {
            *copiaUlt = (*copiaUlt)->ant; // *copiaUlt = p-> ant;
            (*copiaUlt)->prox = NULL; // p->ant->prox = NULL;
        }
        delete(p);
